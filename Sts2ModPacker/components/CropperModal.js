...
export default function CropperModal({ visible, image, onClose, onSave }) {
  ...
  // Calculate min scale: image must cover the crop box
  React.useEffect(() => {
    if (selectedCard && image?.uri) {
      Image.getSize(image.uri, (w, h) => {
        // Image fitted into the mask-sized wrapper (maintaining aspect ratio)
        let renderW, renderH;
        if (w / h > maskW / maskH) {
          renderW = maskW;
          renderH = maskW * (h / w);
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt) => {
        ...
        const applyPanWithBounds = (dx, dy) => {
          ...
          const { w: rw, h: rh } = imageSizeRef.current;
          const maxX = Math.max(0, (rw * s - maskW) / 2);
          const maxY = Math.max(0, (rh * s - maskH) / 2);
  ...
  const applyPanWithBoundsStandalone = (dx, dy) => {
    ...
    const s = lastScale.current;
    const { w: rw, h: rh } = imageSizeRef.current;
    const maxX = Math.max(0, (rw * s - maskW) / 2), maxY = Math.max(0, (rh * s - maskH) / 2);
    newX = Math.max(-maxX, Math.min(maxX, newX));
    newY = Math.max(-maxY, Math.min(maxY, newY));
    lastPan.current.x = newX; lastPan.current.y = newY;
    pan.setValue({ x: newX, y: newY });
    ...
    <Modal visible={visible} animationType="slide">
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.cropContainer} {...panResponder.panHandlers}>
          <Animated.View style={[
            { width: maskW, height: maskH, justifyContent: 'center', alignItems: 'center' },
