...
function DuplicateBindingModal({ visible, stagingImages, cardsData, onResolve, onCancel }) {
  ...
  // Atlas preview calculations
  const atlasKey = cardForPreview?.atlas;
  const aDims = atlasDims[atlasKey] || { w: 4032, h: 4032 };
  const prevSz = 40;
  const prevScale = cardForPreview ? (prevSz / Math.max(cardForPreview.w, cardForPreview.h)) : 0;
            ...
            <>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom: 15 }}>
                ...
                <View style={{ alignItems: 'center', flex: 1 }}>
                  ...
                  {cardForPreview && cardForPreview.atlas && prevScale > 0 ? (
                    <View style={{ width: 150, height: 150, borderRadius: 8, overflow: 'hidden', backgroundColor: '#EEE' }}>
                      <Image
                        source={{ uri: FileSystem.documentDirectory + 'root/' + cardForPreview.atlas }}
                        style={{
                          width: aDims.w * prevScale,
                          height: aDims.h * prevScale,
                          position: 'absolute',
                          left: -cardForPreview.x * prevScale,
                          top: -cardForPreview.y * prevScale,
                        }}
                      />
                    </View>
                  ) : (
