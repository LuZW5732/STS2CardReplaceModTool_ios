...
function DuplicateBindingModal({ visible, stagingImages, cardsData, onResolve, onCancel }) {
  ...
  useEffect(() => {
    if (currentIndices.length > 0 && selectedImgIdx !== null && selectedImgIdx < currentIndices.length) {
      const img = stagingImages[currentIndices[selectedImgIdx]];
      if (img) {
        Image.getSize(img.uri, (w, h) => setUserImageDim({ w, h }), () => setUserImageDim(null));
      }
    }
  ...
  const handleConfirm = () => {
    ...
    // Keep only the selected image's binding, unbind others
    const changes = [];
  ...
  const advanceToNext = () => {
    ...
    // All resolved
    onResolve(null); // signal all done
  };
  ...
  // Parse the key back to display name
  const keyDisplay = currentKey ? currentKey.split('|||').slice(0, 2).join(' / ') : '';

  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 15 }}>
        <View style={{ backgroundColor: '#FFF', borderRadius: 15, maxHeight: '90%', padding: 20 }}>
            ...
            <View style={{ alignItems: 'center', flex: 1 }}>
              ...
              {selectedImg?.binding && (
                <Text style={{ fontSize: 10, color: '#8A7E81', marginTop: 2 }}>
                  模板: {selectedImg.binding.cardName} {selectedImg.binding.rect?.w}x{selectedImg.binding.rect?.h}
                </Text>
              )}
            </View>
          </View>
