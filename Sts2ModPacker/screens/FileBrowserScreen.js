  ...
  const executeFileAction = async (action, file) => {
    try {
      if (action === '删除') {
        ...
        try {
          const zipBase64 = await FileSystem.readAsStringAsync(file.path, { encoding: 'base64' });
          ...
          for (const [filename, zipEntry] of entries) {
            if (zipEntry.dir) {
              dirs.push(outDir + filename);
            } else {
              fileTasks.push({ filename, zipEntry });
            }
          }
          ...
          // Ensure parent dirs for files (collect unique parent dirs)
          const parentDirs = new Set();
          for (const { filename } of fileTasks) {
            const parts = filename.split('/');
            parts.pop();
            if (parts.length > 0) parentDirs.add(outDir + parts.join('/'));
          }
          await Promise.all([...parentDirs].map(d => ensureDir(d)));

          // Extract and write all files in parallel batches (batch size 10 to avoid memory pressure)
          const BATCH = 10;
          let count = 0;
          for (let i = 0; i < fileTasks.length; i += BATCH) {
            ...
            await Promise.all(
              batchResults.map(({ filename, contentBase64 }) =>
                FileSystem.writeAsStringAsync(outDir + filename, contentBase64, { encoding: 'base64' })
              )
            );
            count += batch.length;
          }
      ...
      {isSelectionMode && (
        <View style={styles.batchBar}>
          <TouchableOpacity onPress={() => {
            ...
            <Text style={{color: '#4A4043', fontWeight: 'bold'}}>全选</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => {
            const newSel = new Set();
            files.forEach(f => {
              if (!selectedFiles.has(f.name)) newSel.add(f.name);
            });
            setSelectedFiles(newSel);
          }} style={{marginRight: 15}}>
            <Text style={{color: '#4A4043', fontWeight: 'bold'}}>反选</Text>
          </TouchableOpacity>
